...
const EMPTY_TRASH_BOOK_DEPENDENCY_SQL: &[&str] = &[
    ...
    "DELETE FROM BOOK_PROJECTION WHERE BOOK_ID IN (SELECT ID FROM BOOK WHERE LIBRARY_ID = ? AND DELETED_DATE IS NOT NULL)",
];
...
const BOOK_DEPENDENCY_SQL: &[&str] = &[
    ...
    "DELETE FROM BOOK_PROJECTION WHERE BOOK_ID = ?",
];
